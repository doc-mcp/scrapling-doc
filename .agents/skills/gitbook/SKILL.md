---
name: gitbook
description: Use this skill when editing GitBook documentation in external environments like text editors, IDEs, or git-synced repositories. Trigger this for creating pages, editing markdown, configuring SUMMARY.md, managing assets, setting up variables, using custom blocks (tabs, hints, steppers), adding frontmatter, or structuring documentation with Git Sync. Apply this whenever the user mentions GitBook editing, markdown documentation, or needs guidance on GitBook syntax, blocks, or project configuration.
---
...
```bash
curl -H "X-API-Key: your-api-key" https://api.example.com/v1/users  <!-- gitleaks:allow -->
```
